...
export class PatientDashboardComponent implements OnInit {
  ...
  onEdit() {
    ...
    let counter = 0;
    this.AllPatients.forEach((elem) => {
      if (elem.patientID === patient.patientID) {
        counter = counter + 1;
      }
    })
    if (counter > 1 && patient.visitID !== null) {
      alert("The visit ID you just entered already exists. Please  try a unique value for each patient");
    }
    else {
